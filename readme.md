# Google Apps Script Runtime Emulator & Edge Gateway

Гібридне середовище виконання Google Apps Script (GAS) на Node.js / TypeScript. Проєкт поєднує локальну емуляцію синхронного API, та двосторонню синхронізацію з Google Sheets / MySQL та інтеграцію з Cloudflare для маршутизації Webhook-запитів. 

## Проблема та рішення

Офіційне хмарне середовище Google Apps Script має суворі обмеження: ліміти часу виконання, обмежену кількість API-запитів та відсутність зручного локального тестування/дебагу.

## Детальний опис проблем

Хмарне середовище Google Apps Script має серйозні обмеження для розробки
- **Неможливість локально тестувати та CI/CD:** Код доводиться деплоїти в хмару Google при кожному білду, що суттєво уповільнює розробку.
- **Жорстокі обмеження виконання:** Google має квоти на виконання скриптів, час виконання їх та кількість їх.
- **Повільне виконаня коду**: Серидовище має **Cold Start** що займає 10-15 секунд перед першим запуском, всі подальші по при їх легкість виконуються в середньому за 3-7 секунди.

### Вирішення проблем
1. **Локальна розорбка та дебаг:** Дозволяє виконувати, тестувати та дебажити GAS-скрипти на локальній машині (через Node.js) без постійного деплоїв у Google.
2. **Синхронна емуляція GAS API:** Повна імітація специфічної синхроної поведінки Google Apps Script (наприклад `UrlFetchApp.fetch`, `CacheService`, `PropertiesService`) без необіхідності переписувати код під `async/wait`
3. **Власна база даних:** Модуль `SpreadsheetApp` перенаправляє операції з таблицями у локальну MySQL БД з підтримкою транзакцій, що запезпечує миттєвий відгук і відсутність лімітів Google Sheets API.
4. **Ізольоване виконання:** Підтримка запуску Google Script (JavaScript) коду у пісочниці (Node.js `vm`), що дозволяє безпечно виконувати зовнішній скрипт у локальному середовищі.
5. **Гібридна маршрутизація (Cloudflare Tunnel):**
  - Коли локальний ПК знаходиться в мережі, CloudflareWorker перенаправляє вхідні `Webhook-запити` на локальний Node.js оточення.
  - Це дозволяє обходити хмарні ліміти GAS і виконувати обробку даних локально, зі значно швидшою роботою, ніж у GAS оточенні.
6. **Двостороння синхронізація (Google Sheets <-> MySQL):**
  - Робота з даними здійснюється через локальну MySQL БД для миттєвого відгуку.
  - Реалізовано механізм синхронізації змін із реальними Google Spreasheets у фоновому режимі через `Google Sheets API`.

### Як це працює? Приклад використання:
* **Коли ПК увімкнений:** Cloudflare тунелює вхідні Webhook-запити від Telegram безпосередньо на локальний Node.js сервер. Бот відповідає **миттєво**, а дані зберігаються у локальній БД та поступово по черзі надходять зміни до `Google Sheets API` у фоновому режимі.
* **Коли ПК вимкнений:** Запити автоматично приймаються хмарою Google Apps Script. Бот продовжує працювати без зупинки 24/7, використовуючи синхронізовані дані з Google Sheets як базу даних.
* **Двостороння синхронізація:** Дані між MySQL та Google Sheets синхронізуються, що запезпечує актуальність стану бота в обох середовищах.

---
## Схема архітектури
![Схема архітектури](./assets/GAS.Emulator.drawio.png)
